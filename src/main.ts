...
const attempts: Record<PlayerId, AttemptRecord[]> = { 1: [], 2: [] };
...
<main class="game-shell">
  <section id="stage" class="stage" aria-label="Curicula-VS">
    ...
    <header class="topbar">
      <div class="brand-block">
        ...
        <div><strong>Curicula-VS</strong><small>Hakikat • Prinsip • Komponen • Perkembangan Kurikulum</small></div>
      </div>
    ...
    <section id="startOverlay" class="overlay">
      <div class="dialog start-dialog">
        ...
        <h1>Curriculum Concept Battle</h1>
        <p class="lead">Setiap babak mempertemukan satu sub-tim dari Tim A dan Tim B. <strong>Urutan soal kedua sisi berbeda</strong>, jawaban dikunci dengan ✊, dan kunci beserta pembahasan tersedia setelah pertandingan selesai.</p>
    ...
    <section id="finishOverlay" class="overlay hidden">
      <div class="dialog result-dialog">
        ...
        <div class="result-actions">
          <button id="reviewButton" class="button primary" type="button">Pembahasan & Kunci</button>
    ...
    <section id="reviewOverlay" class="overlay review-overlay hidden">
      <div class="dialog review-dialog">
        <div class="review-head">
          <div><span class="eyebrow">PEMBAHASAN & KUNCI</span><h2>Review Jawaban Curicula-VS</h2></div>
          <button id="closeReviewButton" class="icon-btn" type="button" aria-label="Tutup pembahasan">✕</button>
        </div>
        <div class="review-tabs">
          <button id="reviewTab1" class="review-tab active" type="button">Tim A</button>
          <button id="reviewTab2" class="review-tab" type="button">Tim B</button>
        </div>
...
const reviewOverlay = must<HTMLElement>("#reviewOverlay");
...
must<HTMLButtonElement>("#reviewButton").addEventListener("click", () => openReview(1));
must<HTMLButtonElement>("#closeReviewButton").addEventListener("click", () => reviewOverlay.classList.add("hidden"));
must<HTMLButtonElement>("#reviewTab1").addEventListener("click", () => openReview(1));
must<HTMLButtonElement>("#reviewTab2").addEventListener("click", () => openReview(2));
...
function startMatch(): void {
  ...
  for (const id of [1,2] as const) {
    for (const sub of teams[id].subteams) sub.score = 0;
    attempts[id] = [];
  }
  reviewOverlay.classList.add("hidden");
...
function renderPlayerQuestion(id: PlayerId): void {
  ...
  must(`#sideStimulus${id}`).textContent = `${q.stimulus} ${q.prompt}`;
  must(`#sidePrompt${id}`).textContent = "";
...
function renderBoolean(id: PlayerId, mount: HTMLElement): void {
  ...
  const yes = document.createElement("button"); yes.type = "button"; yes.className = "boolean-option true"; yes.dataset.value = "true"; yes.innerHTML = `<b>☝️ <i>A</i></b><span>BENAR</span>`; yes.addEventListener("click", () => selectBoolean(id, true));
...
function renderMulti(id: PlayerId, q: PreparedMulti, mount: HTMLElement): void {
  ...
  q.options.forEach((text, index) => {
    ...
    btn.innerHTML = `<b class="multi-letter">${letter(index)}</b><span class="check-box">✓</span><span>${escapeHtml(text)}</span>`;
...
function selectBoolean(id: PlayerId, value: boolean): void {
  ...
  setLockState(id, `${value ? "A • BENAR" : "B • SALAH"} • BELUM DIKUNCI`, false);
  setGestureMessage(id, `${value ? "A • BENAR" : "B • SALAH"} dipilih • ✊ untuk KUNCI`, 0);
}

function lockBoolean(id: PlayerId): void {
  const q = currentQuestionFor(id); if (!canSubmit(id) || q?.type !== "boolean") return;
  if (typeof players[id].answer !== "boolean") { setGestureMessage(id, "Pilih ☝️ A = BENAR atau ✌️ B = SALAH terlebih dahulu", 0); return; }
...
function toggleMulti(id: PlayerId, index: number): void {
  ...
  const selectedText = [...set].sort((a,b)=>a-b).map(letter).join(", ");
  setLockState(id, selectedText ? `TERPILIH: ${selectedText} • BELUM DIKUNCI` : "BELUM ADA PILIHAN", false);
  setGestureMessage(id, selectedText ? `Terpilih ${selectedText} • gesture lagi untuk batal • ✊ KUNCI` : "☝️ A • ✌️ B • 🤟 C • ✋ D", 0);
}
...
function finalizePlayerSubmission(id: PlayerId, correct: boolean, customScore?: number, detail?: string): void {
  ...
  const q = currentQuestionFor(id);
  if (q) attempts[id].push({ question: q, answer: cloneAnswer(p.answer), score: gained, correct, detail });
...
function renderWaiting(id: PlayerId): void {
  ...
  const waitingText = chapterTimedOut
    ? "Waktu 7 menit untuk babak ini telah habis. Skor jawaban yang sudah dikunci tetap disimpan. Bersiap menuju babak berikutnya…"
    : `Semua soal untuk sub-tim ini sudah selesai. Skor telah disimpan. ${chapterDone[id === 1 ? 2 : 1] ? "Bersiap menuju babak berikutnya…" : "Tunggu sub-tim lawan menyelesaikan soal mereka."}`;
  must(`#sideStimulus${id}`).textContent = waitingText;
  must(`#sidePrompt${id}`).textContent = "";
...
function cloneAnswer(answer: unknown): unknown {
  if (Array.isArray(answer)) return [...answer];
  if (answer && typeof answer === "object") return { ...(answer as Record<string,string>) };
  return answer;
}

function openReview(id: PlayerId): void {
  reviewPlayer = id;
  must("#reviewTab1").classList.toggle("active", id === 1);
  must("#reviewTab2").classList.toggle("active", id === 2);
  must("#reviewTab1").textContent = teams[1].name;
  must("#reviewTab2").textContent = teams[2].name;
  renderReview(id);
  reviewOverlay.classList.remove("hidden");
}

function renderReview(id: PlayerId): void {
  const attemptById = new Map(attempts[id].map(a => [a.question.id, a]));
  const rows: string[] = [];
  let answered = 0;
  let perfect = 0;
  for (let chapter = 1; chapter <= 4; chapter += 1) {
    const chapterQuestions = sequences[id][chapter - 1] ?? [];
    chapterQuestions.forEach((q, index) => {
      const attempt = attemptById.get(q.id);
      if (attempt) { answered += 1; if (attempt.correct) perfect += 1; }
      rows.push(reviewCard(q, attempt, chapter, index + 1));
    });
  }
  must("#reviewSummary").innerHTML = `<strong>${escapeHtml(teams[id].name)}</strong><span>${answered}/35 dijawab</span><span>${perfect} jawaban sempurna</span><span>${teamTotal(id)} poin</span>`;
  must("#reviewList").innerHTML = rows.join("");
}

function reviewCard(q: PreparedQuestion, attempt: AttemptRecord | undefined, chapter: number, number: number): string {
  const status = !attempt ? "Tidak dijawab" : attempt.correct ? "Benar/Sempurna" : "Belum sempurna";
  const statusClass = !attempt ? "unanswered" : attempt.correct ? "correct" : "wrong";
  const questionText = `${q.stimulus} ${q.prompt}`;
  return `<article class="review-card ${statusClass}">
    <div class="review-card-head"><span>Babak ${chapter} • Soal ${number}</span><b>${status}${attempt ? ` • +${attempt.score}` : ""}</b></div>
    <p class="review-question">${escapeHtml(questionText)}</p>
    <div class="review-answer-grid">
      <div><small>Jawaban pemain</small><strong>${formatUserAnswer(q, attempt?.answer)}</strong></div>
      <div><small>Kunci jawaban</small><strong>${formatCorrectAnswer(q)}</strong></div>
    </div>
    <div class="review-explanation"><small>Pembahasan</small><p>${escapeHtml(q.explanation)}</p></div>
  </article>`;
}

function formatUserAnswer(q: PreparedQuestion, answer: unknown): string {
  if (answer === undefined || answer === null) return "— Tidak dijawab —";
  if (q.type === "single") {
  ...
  return q.pairs.map(pair => {
    const rightId = map[pair.id];
    const target = q.pairs.find(p => p.id === rightId);
    return `${escapeHtml(pair.left)} → ${escapeHtml(target?.right ?? "—")}`;
  }).join("<br>");
}
...
function handleGesture(id: PlayerId, frame: HandFrame, now: number): void {
  ...
  if (q.type === "multi") {
    if (fist) {
      handleHeldGesture(id, "lock-multi", now, "✊ tahan untuk KUNCI", () => submitMulti(id), APP_CONFIG.lockHoldMs);
      return;
    }
    const choice = classifySingleChoiceGesture(frame.landmarks);
    handleHeldGesture(id, choice ? `multi-${choice.index}` : "", now, choice ? `${choice.emoji} → ${letter(choice.index)} (toggle)` : "☝️ A • ✌️ B • 🤟 C • ✋ D • ✊ KUNCI", () => toggleMulti(id, choice!.index));
  }
}
